    ...
    case 3:
      switch (pat[0]) {
        case 'SENSOR':
          switch (pat[1]) {
            case 'CLOCK':
              if (appSensors.Clock.read)
                return [pat[0],pat[1],appSensors.Clock.read()]
              break;
          }
      }
      break;
...
function chatUpdate () {
  ...
  botui = new BotUI('chat-bot',{callback:function (msg) {
      ...
      if (msg.content) {
        if (msg.human) nextChatHistory.push({kind:"answer", delay:100, content:msg.content});
        else nextChatHistory.push({kind:"message", delay:100, content:msg.content});
      }
    }});
  chatRefresh();
}
    ...
    loadjs(url,function (ev) { 
        if (Jam.jam) {
          ...
          if (typeof window[aclass] == 'function') {
            Jam.jam.compileClass(aclass,window[aclass],{verbose:1})
          };
...
function loadClassFromFile(path, file, cb, onerror) {
  ...
  readFile(path, file, function (data) {
    // log(data)
    if (Jam.jam) {
      Jam.jam.compileClass(aclass,data,{verbose:1});
      if (_classes[aclass]==undefined) {
        _classes[aclass]=false;
      }
      if (cb) cb(aclass);
...
function chatQuestion (id,question, action, callback, timeout) {
  ...
  var actions=[],timer;
  chatAction({kind:'message', delay: 100, content: id+': '+question });
  ...
  if (action instanceof Array) {
    ...
    chatAction({kind:'button', delay: 100, action: action});
    chatAction({kind:'checkpoint', callback:callback, timer:timer});
  } else if (typeof action == 'object') {
    // Value or text
    chatAction({kind:'value', delay: 100, action: action});
    chatAction({kind:'checkpoint', callback:callback, timer:timer});  
  } 
...
function chatMessage (id,message) {
  chatAction({kind:'message', delay: 100, content: id+': '+message });
  if (chatActive) chatRefresh();
}
...
function start(next) {
  ...
  Jam.jam.extend([2,3],'question',function (id, question, choices, callback, timeout) {
    ...
    chatQuestion (id,question, choices, function (res) {
      process.callback(callback,[res.value]);
      process.wakeup();
      Jam.jam.schedule();
    },timeout);
