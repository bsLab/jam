 ...
 **  Server, FS FIFO communication, limited to 2 connections (node.js bug)
 **  Data transfer: JSON
 **  ==============================================
 *
 *
...
var Server = function(options) {
  ...
  // Number of channels
  this.channels=Math.min(options.channels||2,2);
...
Server.prototype.init = function () {
  ...
  function receiver (i) {
      ...
      if (!Fs.existsSync(pathO)) {
        Io.out('[BFIS] Cannot open ' + pathO+': not existing!');          
        return;
      }
      if (!Fs.existsSync(pathI)) {
        Io.out('[BFIS] Cannot open ' + pathI+': not existing!');          
        return;
      }
      ...
      function listen () {
        ...
        Io.out('[BFIS] Listening on ' + pathI+' in mode '+self.mode+' '+(self.keepalive?'KEEPALIVE':''));
        ...
        chI.on('data',function (chunk) {
          ...
          Io.log(((log+self.verbose)<2)||('[BFIS] Received: ' + chunk.length));
  ...
  for (var i=0;i<this.channels;i++) {
    ...
    try {receiver(i);} catch (e) {Io.out('[BFIS] Error: '+e)};
  }
