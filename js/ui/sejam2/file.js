...
FS = {
  ...
  loadSync : function(dir,file,mimetype) {
    if (mimetype != 'binary') mimetype='text';
    var result = Utils.POST(FS.url,{
          command:  'load',
          dir:  dir,
          file: file,
          mimetype : mimetype,
...
function loadFilePoly (cb,binary) {
  if (Config.workdir) {
    // Try WEX service?
    return FS.API.loadFile(binary?'binary':'text',function (result,file,dir) {
      if (Utils.isError(result)) {
        Config.workdir='';
        return nativeDialog();
      }
      if (cb) cb(result,file,dir);
    })
  }
...
function saveFilePoly (data, filename, mimetype, callback) {
    if (Config.workdir) {
      // Try WEX service?
...
FS.API.loadFilePoly=loadFilePoly;
FS.API.saveFilePoly=saveFilePoly;
